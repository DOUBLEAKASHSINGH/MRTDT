...
import { UploadCloud, CheckCircle, Activity, FileText, Download, LogOut, ArrowRight, Lock, Mail, Loader2, Database, ShieldAlert, BookOpen, Stethoscope, Search } from 'lucide-react';
...
export default function MedicalTranslatorApp() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatusMsg(`Success: ${file.name} indexed!`);
  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4 relative overflow-hidden">
        {/* Bright Hero Background Elements */}
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="text-center mb-10">
            <h1 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-teal-500 drop-shadow-sm mb-3 tracking-tight">
              MRT AI
            </h1>
            <p className="text-gray-600 text-lg">Medical Research Translator</p>
          </div>

          <div className="bg-white border border-gray-200 p-8 rounded-3xl shadow-xl transition-all duration-300">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">
              {authMode === "signin" ? "Welcome Back" : "Create your Account"}
            </h2>
            
            {authError && (
              <div className={`p-4 mb-6 text-sm font-medium rounded-xl border ${authError.includes("created") ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                {authError}
              </div>
            )}
            ...
            <form onSubmit={handleAuth} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    required
                    className="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl pl-10 pr-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all shadow-inner"
              ...
              <button 
                ...
                className="w-full mt-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg hover:shadow-blue-500/30 flex justify-center items-center disabled:opacity-60"
              >
                {authLoading ? <Loader2 className="animate-spin h-6 w-6" /> : (authMode === "signin" ? "Sign In" : "Sign Up")}
              </button>
            </form>
            ...
            <div className="mt-8 text-center border-t border-gray-100 pt-6">
              ...
              >
                {authMode === "signin" ? "Don't have an account? Sign up for free" : "Already have an account? Log in"}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#F3F4F6] text-gray-900 font-sans selection:bg-blue-200">
      ...
      <nav className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-red-600 hover:bg-red-50 transition-all px-4 py-2 rounded-lg border border-transparent hover:border-red-100"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm relative overflow-hidden">
            ...
            <p className="text-sm text-gray-500 mb-5">Add raw PDF papers to the local ChromaDB for the agents to analyze.</p>
            ...
            >
              ...
              <span className="text-base font-semibold text-gray-600 group-hover:text-blue-700 transition-colors">
